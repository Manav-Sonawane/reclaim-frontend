...
export default function SmartSearchChat({ user }: { user: any }) {
  ...
  if (!isOpen) {
      return (
          <Button 
            ...
            className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-xl bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 z-50 flex items-center justify-center transition-transform hover:scale-105"
          >
  ...
  return (
    <div className="fixed bottom-6 right-6 w-96 max-w-[calc(100vw-3rem)] h-[600px] max-h-[calc(100vh-6rem)] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 z-50 flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-200">
        {/* Header */}
        <div className="p-4 bg-linear-to-r from-blue-600 to-indigo-600 flex justify-between items-center text-white">
                    ...
                    <div className={`max-w-[85%] rounded-2xl p-3 text-sm ${
                        ...
                        {msg.data?.items && msg.data.items.length > 0 && (
                            <div className="mt-3 space-y-2">
                                ...
                                <Link href="/" className="block text-center text-xs text-blue-600 hover:underline mt-1">
                                    View all results
                                </Link>
                            </div>
                        )}
        ...
        <form onSubmit={handleSubmit} className="p-3 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex gap-2">
            <Input 
                ...
                autoFocus
            />
            <Button type="submit" disabled={!query.trim() || loading} className="rounded-full h-10 w-10 shrink-0 p-0">
